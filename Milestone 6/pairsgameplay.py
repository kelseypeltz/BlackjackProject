...
class GamePlay:
    ...
    def __init__(self, deckContent, initialNumberOfCard, winningPoints, doubleVariation):
        ...
        # describe which doubles are allowed 
        self.doubleVariation = doubleVariation
    ...
    def GameProceed (self, gameplay, action, winningPoints, dealerCriticalPointsToStick, doubleVariation):
        ...
        if (action == 0):
         # Check for split condition
            if len(gameplay.playerCards) == 2 and gameplay.playerCards[0][0] == gameplay.playerCards[1][0]:
                # Split the hand
                hand1 = [gameplay.playerCards[0], gameplay.DrawACard()]
                hand2 = [gameplay.playerCards[1], gameplay.DrawACard()]
    
                # Calculate points and usable ace for each hand
                hand1Points, hand1UsableAce = gameplay.CalculateCardPoints(hand1, winningPoints)
                hand2Points, hand2UsableAce = gameplay.CalculateCardPoints(hand2, winningPoints)
    
                # Play each hand separately
                hand1Result = gameplay.GameProceed(GamePlay(gameplay.deckContent, 0, winningPoints, doubleVariation), 0, winningPoints, dealerCriticalPointsToStick, doubleVariation)
                hand2Result = gameplay.GameProceed(GamePlay(gameplay.deckContent, 0, winningPoints, doubleVariation), 0, winningPoints, dealerCriticalPointsToStick, doubleVariation)
    
                # Calculate rewards for each hand
                if hand1Result[3] == -1 or hand1Result[0] < hand2Result[0]:
                    reward = -2
                elif hand1Result[3] == 1 or hand1Result[0] > hand2Result[0]:
                    reward = 2
                else:
                    reward = 0
            else:
                self.playerCards.append(self.DrawACard())
                playerCardPoints, playerCardUsableAce = self.CalculateCardPoints(self.playerCards, winningPoints)
    
                # check if the player busts
                if playerCardPoints > winningPoints:
                    reward = -1
            
                # handle permitted doubles variations
                elif action == 2 and doubleVariation == 1 and len(gameplay.playerCards) == 2:
                ...
                elif action == 2 and doubleVariation == 2 and len(gameplay.playerCards) == 2 and (playerCardPoints == 9 or playerCardPoints == 10 or playerCardPoints == 11):
                    gameplay.playerCards.append(gameplay.DrawACard())
                    playerCardPoints, playerCardUsableAce = gameplay.CalculateCardPoints(gameplay.playerCards, winningPoints)
                ...
                elif action == 2 and doubleVariation == 3 and len(gameplay.playerCards) == 2 and (playerCardPoints == 10 or playerCardPoints == 11):
                    gameplay.playerCards.append(gameplay.DrawACard())
                    playerCardPoints, playerCardUsableAce = gameplay.CalculateCardPoints(gameplay.playerCards, winningPoints)
            ...
            # if player chooses to stick, then dealer's turn to draw card
                ...
                # dealer only sticks when his points >= dealerCriticalPointsToStick
            while dealerCardPoints < dealerCriticalPointsToStick:
                gameplay.dealerCards.append(gameplay.DrawACard())
                dealerCardPoints, dummy = gameplay.CalculateCardPoints(gameplay.dealerCards, winningPoints)
